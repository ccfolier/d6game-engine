...
# ライフ
あなたのライフは、例えば戦闘ダメージを受けたり、アクシデントによって減少していきます。ライフの値が0以下になった場合、行動不能となり、パーティの全員が行動不能となった場合はゲームオーバーとなります。ライフの初期値は、六面ダイスをひとつ振った結果を10に足した合計値で決定します。
# 命運
あなたは判定に成功したり、重要な謎を明らかにしたり、戦闘に勝利した際に命運を獲得することがあります。獲得した命運の値を消費することで、重要な局面で判定を有利にすることができます。命運は原則としてゲーム毎にリセットされ、初期値は0です。
# 行為判定
...
# 戦闘
戦闘を行う場合には、ライフの値を賭けて（命を賭けて）ゲームマスターやプレイヤー同士が勝負を行います。多くの場合、ゲーム中に手に入れたアイテムや情報を使って最後にゲームマスターの用意した敵役を打ち倒すことがゲームの目標になります。
