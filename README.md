...
# 命運
あなたは判定に成功したり、重要な謎を明らかにしたり、戦闘に勝利した際に命運を獲得することがあります。獲得した命運の値を消費することで、重要な局面で判定を有利にすることができます。この値は通常引き継がずにゲーム毎にリセットされ、ゲーム開始時は通常0からはじまります。
# 行為判定
...
## 命運を賭けた判定
判定時を行う前に、命運を消費することで、消費した命運の値1につき役の得点を+1することができます。命運を消費して役を操作した場合、判定成功時の命運獲得は行うことができません。
## 役の種類と得点
### ノーハンド（-5点）
役が作れない（ファンブル）
例 ○○○○○
